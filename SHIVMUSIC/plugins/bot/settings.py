...
from SHIVMUSIC.utils.decorators.admins import ActualAdminCB
from SHIVMUSIC.utils.decorators.language import language, languageCB
from SHIVMUSIC.utils.inline.settings import (
...
from SHIVMUSIC.utils.html_builder import build_welcome_html
...
async def settings_cb(client, CallbackQuery, _):
    ...
    return await CallbackQuery.edit_message_text(
        _["setting_1"].format(app.mention, CallbackQuery.message.chat.id, CallbackQuery.message.chat.title),
        reply_markup=InlineKeyboardMarkup(buttons),
    )

@app.on_callback_query(filters.regex("^settingsback_text_legacy$") & ~BANNED_USERS)
@languageCB
async def settings_back_markup(client, CallbackQuery: CallbackQuery, _):
    ...
    if CallbackQuery.message.chat.type == ChatType.PRIVATE:
        ...
        return await CallbackQuery.edit_message_text(
            build_welcome_html(
                CallbackQuery.from_user.mention, app.mention, app.username
            ),
            reply_markup=InlineKeyboardMarkup(buttons),
...
@app.on_callback_query(filters.regex(r"^(SEARCHANSWER|PLAYMODEANSWER|PLAYTYPEANSWER|AUTHANSWER|ANSWERVOMODE|VOTEANSWER|PM|AU|VM)$") & ~BANNED_USERS)
@languageCB
async def without_Admin_rights(client, CallbackQuery, _):
    command = CallbackQuery.matches[0].group(1)
    if command == "SEARCHANSWER": return await CallbackQuery.answer(_["setting_2"], show_alert=True)
    if command == "PLAYMODEANSWER": return await CallbackQuery.answer(_["setting_5"], show_alert=True)
    if command == "PLAYTYPEANSWER": return await CallbackQuery.answer(_["setting_6"], show_alert=True)
    if command == "AUTHANSWER": return await CallbackQuery.answer(_["setting_3"], show_alert=True)
    if command == "VOTEANSWER": return await CallbackQuery.answer(_["setting_8"], show_alert=True)
...
async def addition(client, CallbackQuery, _):
    ...
    if mode == "M":
        final = current - 2
        if final <= 0: return await CallbackQuery.answer(_["setting_11"], show_alert=True)
        if final <= 2: final = 2
        await set_upvotes(CallbackQuery.message.chat.id, final)
    else:
        final = current + 2
        if final >= 17: return await CallbackQuery.answer(_["setting_12"], show_alert=True)
